...
function basicAnalyzeRows(rows: Record<string, unknown>[]) {
  ...
  const analyzed = rows.map((row, index) => {
    ...
    if (!normalizedPhone || !/^\+2519\d{8}$/.test(normalizedPhone)) {
      ...
      aiNote = rawCell
        ? `Could not safely extract a valid phone from: ${rawCell}`
        : "Missing phone value";
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (authHeader) {
      ...
      if (user) {
        const staffResp = await supabaseUser
          .from("staff_profiles")
          .select("role, active")
          .eq("user_id", user.id)
          .maybeSingle();
    ...
    if (action === "DISCOVER" || (mode === "file" && headers?.length)) {
      ...
      const fallbackGiftColumn =
        headers.find((h: string) => /gift|reward|type/i.test(h)) || null;

      try {
        const aiResult = await callGemini(
          `You are analyzing spreadsheet headers for a one-time gift campaign in Ethiopia. Return strict JSON: {"phoneColumn": string, "rewardTypeColumn": string | null, "suggestedRewardType": string | null, "confidence": number, "reasoning": string}. Important: some files may have a single column like "phone no" and cells like "251911510367 - Misbah Ali Mohammed". Headers: ${JSON.stringify(headers)} Sample rows: ${JSON.stringify(sampleRows)}`
        );
        ...
        return json({
          ...
          confidence: 0.72,
          reasoning: "Fallback header detection used.",
        });
      }
    }
    ...
    if (action === "ANALYZE" || action === "analyze") {
      ...
      try {
        const aiResult = await callGemini(
          `You are reviewing import rows for a one-time gift campaign. Return strict JSON: {"summary": string, "rows": [{"index": number, "status": "valid" | "invalid" | "duplicate" | "suspicious", "reason": string, "aiNote": string}]}. Do not invent phone numbers. Input rows: ${JSON.stringify(fallback.rows.slice(0, 500))}`
        );
        ...
        const mergedRows = fallback.rows.map((row) => {
          ...
          return {
            ...row,
            status: ["valid", "invalid", "duplicate", "suspicious"].includes(ai.status)
              ? ai.status
              : row.status,
    ...
    if (action === "IMPORT" || action === "import") {
      ...
      // IMPORTANT:
      // Use the USER-SCOPED client for RPCs that depend on auth.uid() / is_admin().
      const batchResp = await supabaseUser.rpc("create_import_batch", {
      ...
      if (batchResp.error) {
        return json({ error: batchResp.error.message }, 400);
      }
      ...
      for (let start = 0; start < importRows.length; start += chunkSize) {
          ...
          normalized_phone:
            ...
            normalizePhone(
              extractPhoneFromText(
                String(row.rawPhone ?? row.phone_raw ?? row.phone ?? "")
              )
            ),
          gift_type:
            row.giftType ??
        ...
        // Direct insert is fine here because we already checked admin in the function.
        const insertResp = await supabaseAdmin.from("import_batch_rows").insert(chunk);

        if (insertResp.error) {
          return json({ error: `Failed adding batch rows: ${insertResp.error.message}` }, 400);
        }
      }

      // IMPORTANT:
      // finalize_import_batch also depends on auth.uid()/is_admin(), so use supabaseUser.
      const finalizeResp = await supabaseUser.rpc("finalize_import_batch", {
        p_batch_id: batchId,
        p_campaign_id: campaignId || null,
      });

      if (finalizeResp.error) {
        return json({ error: finalizeResp.error.message }, 400);
      }
